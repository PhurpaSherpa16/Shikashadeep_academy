...
export default function ImageCard({ item, handleOpenViewer, index }) {
    return (
        <div key={item.id} className="group cursor-pointer bg-white rounded-lg overflow-hidden border border-gray-100 shadow-sm hover:shadow-xl transition-all duration-500 hover:-translate-y-2"
            ...
            <div className="aspect-4/3 relative overflow-hidden">
                ...
                <div className="absolute bottom-4 left-4 right-4">
                    <div className="bg-white/90 backdrop-blur p-4 rounded-lg transform translate-y-2 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500 shadow-lg">
                        <h3 className="font-bold text-gray-900 truncate">{item.post.title}</h3>
                        <p className="text-xs text-gray-500 truncate mt-1">{item.post.caption}</p>
                        <div className='flex gap-1'>
                            {item?.galleryPostTags?.map((t, i)=>(
                            <button key={i} className='text-black/60 text-sm'>#{t.tag.name}</button>
                        ))}
                        </div>
                    </div>
                </div>
