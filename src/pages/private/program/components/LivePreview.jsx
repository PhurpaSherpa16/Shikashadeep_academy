...
export default function LivePreview({ itemLoading, itemError, filteredPrograms, getAllItemResponse}) {
    ...
    return (
        <div className="lg:col-span-2 space-y-6 h-fit">
            ...
            {itemLoading ? (
                // later change with component skelaton
                <div>
                    <p>Loading...</p>
                </div>
            ) : itemError ? (
                <div className="h-64 rounded-2xl border-2 border-dashed border-red-100 bg-red-50/30 flex flex-col items-center justify-center gap-3 p-8 text-center animate-in zoom-in-95 duration-500">
                    ...
                    <div>
                        ...
                        <p className="text-red-500/80 text-sm max-w-xs">{itemError}</p>
                    </div>
                    <button onClick={() => getAllItemResponse()} className="text-sm font-bold text-red-600 hover:text-red-700 transition-colors bg-white px-4 py-2 rounded-lg shadow-sm border border-red-100">
                        Refresh Dashboard
                    </button>
                </div>
