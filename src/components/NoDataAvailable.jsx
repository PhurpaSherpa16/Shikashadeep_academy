...
export default function NoDataAvailable({ message, link, linkText, height="h-64", defaultIcon}) {
    const icon = defaultIcon || <Plus className="size-4"/>
    return (
        ...
        rounded-lg border-dotted`}>
            <p className="text-gray-500 flex flex-col items-center gap-4">
                {message}
                {link && linkText && (
                    <Link to={link} className="group text-blue-dark font-medium flex items-center gap-2
